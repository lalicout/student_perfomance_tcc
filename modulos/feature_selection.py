...
def selecionar_ordinais_relevantes(df, variaveis_ordinais, target):
    ...
    # Exibe a tabela formatada dos resultados selecionados (para uso interativo)
    print(f"Variáveis ordinais relevantes para explicabilidade do atributo '{target}'\n  com base nos testes estatísticos:")
    # Formata números para exibição
...
def comparar_modelos_regressao(modelos, nomes):
    ...
    for nome, modelo in zip(nomes, modelos):
        try:
            ...
            bic = modelo.bic
            llf = modelo.llf
            r2 = modelo.rsquared if hasattr(modelo, 'rsquared') else None
            r2_adj = modelo.rsquared_adj if hasattr(modelo, 'rsquared_adj') else None
